...
Import-Module (Join-Path -Path $PSScriptRoot -ChildPath 'SullTec.Common.PowerShell' -AdditionalChildPath 'SullTec.Common.psd1')
#####################
# Declare Functions #
#####################
...
class GitRepo {
    ...
    [void]InvokeInitialize() {
        if ((Get-ChildItem).Count -eq 0) {
            ...
            $this.Display()
            Write-Log "$($this.Name) is already initialized." -Title 'Init'
        }
    }
    [void]InvokeCheckout() {
        $this.Display()
